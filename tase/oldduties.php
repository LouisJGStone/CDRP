                    ...
                    <div class="portlet-body">
                      ...
                      <table class="table table-hover table-advance table-bordered table-striped">
                        ...
                        <tbody>
                          ...
                          $day = $_GET["day"];
                          $res = getConnection()->query("SELECT * FROM Duties WHERE Day='".$day."'");

                          if ($res->num_rows > 0) {

                            while ($row = $res->fetch_assoc()) {

                              echo "<tr><td>".$row["Callsign"]."</td><td>".getShortRank($row["UserID"])." ".getFirstName($row["UserID"])[0]." ".getLastName($row["UserID"])."</td>
                              <td>".getDivision($row["UserID"])."</td><td>".$row["Role"]."</td><td>".$row["Vehicle"]."</td>".'<td><a href="javascript:popup_assign('.$row["UserID"].','.$day.')" class="btn default green-stripe btn-xs"> Assign </a><a href="javascript:;" class="btn default red-stripe btn-xs"> Edit </a></td>'."</tr>";
...
function popup_assign(day, user) {
  ...
  var url = "popup_assignshift.php?day="+user+"&user=" + day;
	var w = 350;
  var h = 450;
